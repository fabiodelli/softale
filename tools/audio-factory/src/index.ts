...
import { FFmpegMixer, VoiceSegment } from './services/FFmpegMixer.js';

// ... (existing imports, keep them)

// =====================================================
// Unified Mixer (3-Stem Support + Sidechaining)
// =====================================================

export interface StudioMixResult {
    mixedPath: string;
    voiceStemPath: string;
}
...
export async function mixUnifiedAudio(
    ...
    console.log('🎛️ Unified Mixing (Voice + Music + Ambience) with Studio Rack...');

    const mixer = new FFmpegMixer(OUTPUT_DIR);

    // 1. Calculate Timestamps
    const voiceSegments: VoiceSegment[] = [];
    let currentTime = script.warmupDuration || 0;
    let phasesDuration = 0;

    for (const p of script.phases) {
        if (voicePaths.has(p.id)) {
            voiceSegments.push({
                path: voicePaths.get(p.id)!,
                startTime: currentTime
            });
        }
        currentTime += p.durationSeconds;
        phasesDuration += p.durationSeconds;
    }

    const totalDuration = phasesDuration + (script.warmupDuration || 0);

    // 2. Delegate to Mixer Service
    const result = await mixer.createStudioMix(
        voiceSegments,
        musicPath,
        ambiencePath,
        {
            outputPath: '', // Auto-generated by mixer
            duration: totalDuration,
            warmupDuration: script.warmupDuration || 0
        }
    );

    if (!result.success || !result.mixedPath) {
        throw new Error(`Mixing failed: ${result.error}`);
    }

    return {
        mixedPath: result.mixedPath,
        voiceStemPath: result.voiceStemPath || ''
    };
}
...
export async function generateStory(brief: StoryBrief) {
    ...
    // 5. Mix Full & Voice Stem
    const mixResult = await mixUnifiedAudio(script, voiceRes.paths, musicPath, ambiencePath);

    // 6. Upload
    await uploadStoryPackage(script, mixResult.mixedPath, {
        music: musicPath,
        ambience: ambiencePath,
        voiceMap: voiceRes.paths,
